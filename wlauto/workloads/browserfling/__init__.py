...
class Browserfling(Workload):
    ...
    def setup(self, context):
        self.testdir = '/'.join([self.device.working_directory, 'browserfling'])
        self.testscript_target = self.testdir + "/browserfling.sh";
        self.testdefs_target = self.testdir + "/defs.sh";
        self.testpage_target = self.testdir + "/testpage"
        ...
        if not self.device.file_exists(self.testscript_target):
            self.testscript_host = context.resolver.get(File(self, "browserfling.sh"))
        ...
        if not self.device.file_exists(self.testpage_target):
            self.testpage_host = context.resolver.get(File(self, "testpage"))
            self.device.push_file(self.testpage_host, self.testpage_target)

        self._kill_browserfling()
        self.device.ensure_screen_is_on()

    def run(self, context):
        args = '-i {} '.format(self.loops)
        cmd = "echo $$>{dir}/pidfile; cd {bindir}; exec ./browserfling.sh {args}; rm {dir}/pidfile"
    ...
    def teardown(self, context):
        print "Leaving browserfling files on target..."
#        self.device.uninstall_executable(self.browserfling_target)
#        self.device.uninstall_executable(self.defs_target)

    def _kill_browserfling(self):
