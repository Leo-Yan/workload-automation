...
class Galleryfling(Workload):
    ...
    def setup(self, context):
        self.testdir = '/'.join([self.device.working_directory, 'galleryfling'])
        self.testscript_target = self.testdir + "/galleryfling.sh"
        ...
        if not self.device.file_exists(self.testscript_target):
            self.testscript_host = context.resolver.get(File(self, "galleryfling.sh"))
        ...
        self._kill_galleryfling()
        self.device.ensure_screen_is_on()

    def run(self, context):
        args = '-i {} '.format(self.loops)
        cmd = "echo $$>{dir}/pidfile; cd {bindir}; exec ./galleryfling.sh {args}; rm {dir}/pidfile"
    ...
    def teardown(self, context):
        print "Leaving galleryfling files on target..."
#        self.device.uninstall_executable(self.galleryfling_target)
#        self.device.uninstall_executable(self.defs_target)

    def _kill_galleryfling(self):
